...
namespace RealEstate_Dapper_UI.Controllers
{
    public class LoginController : Controller
    {
        ...
        public async Task<IActionResult> Index(CreateLoginDto loginDto)
        {
            ...
            var response = await client.PostAsync("http://localhost:7240/api/Logins", content);
            if (response.IsSuccessStatusCode)
            {
                var jsonData = await response.Content.ReadAsStringAsync();
                var tokenModel=JsonSerializer.Deserialize<JsonResponseModel>(jsonData, new JsonSerializerOptions
                {
